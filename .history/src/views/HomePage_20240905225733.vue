...
const addWaypoint = (e: any) => {
  ...
  set(waypointsRef, newWaypoint);
  waypoints.value.push(newWaypoint);
};

const syncWaypointsToFirebase = () => {
  // Format waypoints as an object with index as keys for Firebase
  const waypointsData = waypoints.value.reduce((acc: any, waypoint, index) => {
    acc[index] = {
      name: waypoint.name,
      lat: waypoint.lat,
      lon: waypoint.lng,  // Use "lon" for longitude as required
    };
    return acc;
  }, {});

  // Update waypoints in Firebase
